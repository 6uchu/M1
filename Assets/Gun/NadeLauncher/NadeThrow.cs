...
public class NadeThrow : MonoBehaviour, IFireStrategy
{
    ...
    public void Fire(WeaponData data)
    {
        ...
        if (hit)
        {
            ...
            {
                // 적 피해 처리
                hit.collider.GetComponent<Enemy>().TakeDamage(data.damage);
            }
            else if (hit.collider.CompareTag("Barricade"))
            {
                // 적 피해 처리
                hit.collider.GetComponent<Barricade>().TakeDamage(data.damage);
            }
        }
    }
